...
func (s *UserBadgeSeeder) getUserBadgeCountTx(tx *sqlx.Tx, userID string) int {
	var count int
...
func (s *UserBadgeSeeder) getUserBadgeCountDB(userID string) int {
	var count int
...
func (s *UserBadgeSeeder) AssignBadgeToUser(userID string, badgeID int64) error {
	var existingID sql.NullInt64
	err := s.db.QueryRow(
		"SELECT id FROM user_badges WHERE user_id = $1 AND badge_id = $2",
		userID, badgeID,
	...
	if err == nil && existingID.Valid {
		PrintWarning(fmt.Sprintf("User ID %s already has badge ID %d", userID, badgeID))
		return nil
	}
	...
	PrintSuccess(fmt.Sprintf("Badge ID %d awarded to user ID %s", badgeID, userID))
	return nil
}

// AssignMultipleBadges assign multiple badges to multiple users
// Format: map[userID][]badgeIDs
func (s *UserBadgeSeeder) AssignMultipleBadges(assignments map[string][]int64) error {
	tx, err := s.db.Beginx()
	if err != nil {
		PrintError(fmt.Sprintf("Failed to start transaction: %v", err))
		return err
	...
	for userID, badgeIDs := range assignments {
		...
		if err != nil || !exists {
			PrintError(fmt.Sprintf("User ID %s not found", userID))
			continue
		}
